...
impl LBFOptimizerQP {
    pub fn new(instance: QPInstance, config: LBFConfig, rng: SmallRng) -> Self {
        assert!(config.n_samples > 0);
        let problem = QPProblem::new(instance.clone());
        Self {
            instance,
            problem,
            config,
    ...
    pub fn solve(&mut self) -> QPSolution {
        ...
        'outer: for item_id in item_placement_order(&self.instance) {
            ...
            while self.problem.item_demand_qtys[item_id] > 0 {
                ...
                match placement {
                    Some((d_transf, _)) => {
                        self.problem.place_item(QPPlacement {
                            item_id: item.id,
                            d_transf,
                        });
                        info!(
                    ...
                    None => {
                        ...
                        self.problem
                            .change_square_side_length(self.problem.square.side_length * 1.1);
                        info!(
