...
fn main() -> Result<()> {
    ...
    let (explore_dur, compress_dur) = match (args.global_time, args.exploration, args.compression) {
        (Some(gt), None, None) => (
            Duration::from_secs(gt).mul_f64(DEFAULT_EXPLORE_TIME_RATIO),
            Duration::from_secs(gt).mul_f64(DEFAULT_COMPRESS_TIME_RATIO),
        ),
        (None, Some(et), Some(ct)) => (Duration::from_secs(et), Duration::from_secs(ct)),
        (None, None, None) => {
            warn!("[MAIN] no time limit specified");
            (
                Duration::from_secs(600).mul_f64(DEFAULT_EXPLORE_TIME_RATIO),
                Duration::from_secs(600).mul_f64(DEFAULT_COMPRESS_TIME_RATIO),
            )
        }
