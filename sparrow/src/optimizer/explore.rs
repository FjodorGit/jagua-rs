...
pub fn exploration_phase(
    ...
    let mut current_width = sep.prob.side_length();
    ...
    let mut solution_pool: Vec<(QPSolution, f64)> = vec![];
    ...
    while !term.kill() {
        ...
        if total_loss == 0.0 {
            ...
            info!(
                "[EXPL] shrinking square by {}%: {:.3} -> {:.3}",
                config.shrink_step * 100.0,
                current_width,
                next_width
            );
            sep.change_square_side_length(next_width);
