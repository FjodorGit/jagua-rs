...
fn main() -> Result<()> {
    ...
    let mut best_s_val = 15000.0;
    let mut best_x1_val = 0.0;
    let mut best_y1_val = 0.0;
    let mut best_x2_val = 0.0;
    let mut best_y2_val = 0.0;
    let mut best_deg1 = 0.0;
    let mut best_deg2 = 0.0;
    for deg in 0..181 {
        let deg1: f64 = 23.629377;
        let deg2: f64 = deg1 + Into::<f64>::into(deg);
        ...
        for (nfp_idx, nfp) in nfps.iter().enumerate() {
            ...
            for edge_idx in 0..n_points {
                ...
                model.add_constr(
                    ...
                    c!((b.x - a.x) * (y2 - y1) + (a.y - b.y) * (x2 - x1) + c_val
                        <= (1.0 - v) * m_22),
                )?;
        ...
        if status == Status::Optimal || status == Status::SubOptimal {
            ...
            if best_s_val > s_val {
                ...
                println!("Solution was worse");
            }
        }
    }
    println!("Best solution found:");
    println!("  s = {:.2}", best_s_val);
    println!("  x1 = {:.2}, y1 = {:.2}", best_x1_val, best_y1_val);
    println!("  x2 = {:.2}, y2 = {:.2}", best_x2_val, best_y2_val);
    println!("  deg1 = {:.2}, deg2 = {:.2}", best_deg1, best_deg2);
